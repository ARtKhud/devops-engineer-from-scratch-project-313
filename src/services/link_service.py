...
class LinkService:
    ...
    def get_all_links(self, skip: int = 0, limit: int = 10) -> list[LinkResponse]:
    ...
    def create_link(self, link_creation_data: LinkCreate) -> Optional[LinkResponse]:
        exsiting_link = self.repo.find_by_short_name(link_creation_data.short_name)
        if not exsiting_link:
            link = self.repo.create_link(link_creation_data)
            return LinkResponse.model_validate(link)
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Short name '{link_creation_data.short_name}' already exists",
        )
